...
class MasterPackage extends React.Component {
	static async getInitialProps(ctx) {
		auth(ctx)
		const { store } = ctx
		const { token } = nextCookie(ctx)
		let props = { 
			showHeader: true, showFooter: true, pageName: "/master-package", 
			page: 1, size: 10, sortBy: "name", orderBy: "desc", keyword: "", token: token, fetch_category: "package"
		}
		try {
	...
	constructor(props) {
		...
		this.state = {
            ...
            _listProduct: props.listPackage,
	...
	UNSAFE_componentWillReceiveProps(nextProps) {
		this.setState({
			...
			_listProduct: nextProps.listPackage
		})
	}
	...
	doPostItem = () => {
		const { onFetch, payloadProductName, payloadProductPrice, payloadProductDescription, payloadProductImage, payloadIsOneTime } = this.state 
		if(payloadProductName && payloadProductPrice && payloadProductDescription && payloadProductImage && payloadIsOneTime) {
			if(!onFetch) {
				let datas = {
					name: payloadProductName,
					price: payloadProductPrice,
					...
					description: payloadProductDescription,
					link: payloadProductImage,
					is_one_time: payloadIsOneTime != "1" ? false : true
				}
				this.setState(
					{ onFetch: true },
					async () => {
	...
	renderModalAddEditItem() {
		const { onFetch, isEdited, showAddEditItemModal, payloadProductName, payloadIsOneTime, payloadProductPrice, payloadProductImage, payloadProductDescription } = this.state
		return (
			<ModalBox 
				title={isEdited ? `Edit - ${payloadProductName}` : 'New Package'} 
				size="lg" 
				className="absolute-center" 
							...
							<Row>
								<Col sm="12" lg="12">
									<Row>
										...
										</Col>
										{
											!isEdited ? 
												<Col lg="12">
													<FormSelect
														withLabel={true}
														labelName="Maximum Participant Quantity"
														labelClassName="font-14 mb-2 text-999999"
														containerClassName="mb-3"
														formId="yourFormIsOneTime" 
														formName="payloadIsOneTime" 
														formSize="sm"
														formPlaceholder="Please select"
														formOptionData={[
															{ id: "0", name: "Participant >=25" },
															{ id: "1", name: "Participant < 25" }
														]}
														formValue={payloadIsOneTime}
														onChange={this.handleChange}
													/>
												</Col> : ""
										}
										...
										<Col sm="6" lg="4">
											<ContentBox title="Package Photo" className="p-1 overflow-auto" titleClass="font-14 mb-2 text-999999">
											{
												payloadProductImage ? 
	...
	renderModalZoomImg() {
		...
		return (
			<ModalBox
				title={zoomImage ? zoomImage.name : "Product Package"}
				size="lg"
				className="absolute-center" 
				bodyClassName="p-0"
				...
				>
					<Row>
						<Col lg="12" className="mb-5">
							<h1 className="text-primary">PACKAGES</h1>
							<span><i className="icon-paperclip mx-1" />Total {_listProduct.total_elements} {_listProduct.total_elements > 1 ? 'Packages' : 'Package'}</span>
						</Col>
...
export default connect(state => state, mapDispatchToProps)(MasterPackage)